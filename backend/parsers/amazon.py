    ...
    try:
        for file in files:
            ...
            try:
                doc = fitz.open(save_path)
                full_text = "".join([page.get_text() for page in doc])
                doc.close()  # Free memory resource immediately
                ...
                if document_type == "Invoice":
                    row_pattern = re.compile(
                        r"(\d+)\.\s+(\d{6})\s+(.*?)\s+INR\s*([0-9,]+\.\d+)(.*?)(?=\d+\.\s+\d{6}|\Z)",
                    ...
                    credit_pattern = re.compile(
                        r"(\d+)\.\s*([A-Z0-9\-]+)\s*([0-9\-]+)\s*(\d{6})\s*(.*?)\s*INR\s*([0-9,]+\.\d+)",
                        re.IGNORECASE | re.DOTALL
                    )
                    credit_matches = credit_pattern.findall(normalized_text)
            ...
            gc.collect()  # Force memory recovery after processing each file
        ...
        output_file = os.path.join(OUTPUT_FOLDER, f"Amazon_Invoice_Output_{timestamp}.xlsx")
