...
#             total_amount = round(
#                 taxable_value + sgst_amount + cgst_amount + igst_amount,
#                 2
#             )
...
# async def upload_zomato(files: List[UploadFile] = File(...),
...
#         OUTPUT_FOLDER,
#         f"Zomato_Invoice_Output_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
#     )
...
# # =====================================================
# # UPLOAD ZOMATO API (MODIFIED WITH AMAZON EXCEL LOGIC)
# # =====================================================
...
#     # 3. Amazon structure ki tarah fixed output path aur old file deletion logic
#     output_file = os.path.join(OUTPUT_FOLDER, "Zomato_Invoice_Output.xlsx")
#     if os.path.exists(output_file):
#         os.remove(output_file)

#     # 4. Save Excel cleanly
#     with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
#         df.to_excel(writer, index=False, sheet_name="Zomato_Invoices")

#     # 5. Perfect auto-fit width formatting using openpyxl
#     wb = load_workbook(output_file)
#     ws = wb.active
...
#     print(f"Zomato Excel Created Successfully: {output_file}")

#     # 6. Database Analytics Log updated
#     if files_count > 0:
#         analytics = UploadAnalytics(
...
#     # 7. File response with strict download headers for the frontend browser
#     return FileResponse(
#         path=output_file,
