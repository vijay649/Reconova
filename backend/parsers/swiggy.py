...
def process_swiggy_pdf(full_path, file_name):
    ...
    try:
        ...
        doc.close()  # Immediately free PyMuPDF document memory
    except Exception as e:
...
async def upload_swiggy(
    ...
    try:
        # Stream file buffers directly to disk to minimize memory consumption
        for file in files:
            safe_filename = os.path.basename(file.filename)
            save_path = os.path.join(UPLOAD_FOLDER, safe_filename)
            ...
            try:
                processed_data = process_swiggy_pdf(save_path, safe_filename)
                results.extend(processed_data)
            except Exception as e:
                print(f"Error processing file {safe_filename}: {str(e)}")

            gc.collect()  # Release garbage collected objects per file loop iteration
        ...
        output_file = os.path.join(
            OUTPUT_FOLDER,
            f"Swiggy_Invoice_Output_{os.getpid()}.xlsx"
        )
        ...
        # Save analytics in DB if user is present
        if files_count > 0 and current_user and hasattr(current_user, 'id'):
            analytics = UploadAnalytics(
                user_id=current_user.id,
                source="swiggy",
        ...
        # Clean output Excel file after download
        ...
        # Cleanup processed temporary upload PDFs
